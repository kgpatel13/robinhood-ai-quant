...
class CoinGeckoProvider:
    ...
    def fetch_daily_bars(
        ...
        daily = prices.groupby("day")["price"].agg(open="first", high="max", low="min", close="last")
        ...
        frame = daily.assign(
            symbol=symbol.upper(), asset_class=asset_class, adj_close=daily["close"],
            dividends=0.0, splits=0.0, source=self.name, ingested_at=datetime.now(UTC),
        )
