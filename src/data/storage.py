...
class ParquetBarStore:
    ...
    def write(self, frame: pd.DataFrame, asset_class: str, symbol: str) -> Path:
        ...
        if path.exists():
            ...
            normalized = normalize_bars(pd.concat([existing, normalized], ignore_index=True))
            normalized = normalized.drop_duplicates(["symbol", "timestamp"], keep="last")
        normalized.to_parquet(path, index=False, compression=self.compression)
