...
def run_phase10_replay(data_root: Path, config: Phase10Config) -> Phase10Result:
    ...
    label_quality = label_quality_analysis(replay)
    artifacts["label_quality_analysis"] = _write_csv(
        label_quality, output / "label_quality_analysis.csv"
    )
    feature_ic = feature_predictiveness(
        replay,
        (
            "trend_fast_slow",
            "trend_50_200",
    ...
    artifacts["feature_predictiveness"] = _write_csv(
        feature_ic, output / "feature_predictiveness.csv"
    )
    feature_stability_result = feature_stability(feature_ic)
    artifacts["feature_stability"] = _write_csv(
        feature_stability_result, output / "feature_stability.csv"
    )
    rank_analysis = cross_sectional_rank_analysis(replay)
    artifacts["cross_sectional_rank_analysis"] = _write_csv(
        rank_analysis, output / "cross_sectional_rank_analysis.csv"
    )
    decay = time_decay_analysis(replay)
    artifacts["time_decay_analysis"] = _write_csv(decay, output / "time_decay_analysis.csv")
    confidence = bootstrap_confidence(replay, samples=config.bootstrap_samples)
    artifacts["bootstrap_confidence"] = _write_csv(confidence, output / "bootstrap_confidence.csv")
    final_decisions = final_promotion_decisions(
        ...
        FinalPromotionRules(
            minimum_recent_average_return=config.final_minimum_recent_average_return,
            minimum_bootstrap_positive_probability=(
                config.final_minimum_bootstrap_positive_probability
            ),
            minimum_cross_sectional_top_quantile_lift=(
                config.final_minimum_cross_sectional_top_quantile_lift
            ),
            minimum_feature_stability_fraction=(config.final_minimum_feature_stability_fraction),
        ),
    )
    artifacts["final_promotion_decisions"] = _write_csv(
        final_decisions, output / "final_promotion_decisions.csv"
    )
    final_signoff = final_research_signoff(final_decisions, leakage, "0.10.4", data_cutoff)
    final_signoff_path = output / "phase10_final_signoff.json"
    final_signoff_path.write_text(json.dumps(final_signoff, indent=2), encoding="utf-8")
    artifacts["final_research_signoff"] = str(final_signoff_path)
    ...
    manifest = {
        "phase": "10.4.0",
        "mode": "final_phase10_research_validation",
        ...
        "research_controls": [
            ...
            "Advanced validation includes label quality, feature predictiveness, "
            "cross-sectional ranking, time decay, and bootstrap confidence.",
